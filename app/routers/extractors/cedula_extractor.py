...
router = APIRouter(prefix="/api/v1/extractor/cedula", tags=["Cedula extractor"], responses={404: {"description": "Not found"}})

barcode_processor = BarcodeProcessor()
dni_extractor_instance = CedulaExtractor(barcode_processor)

@router.post("/front", status_code=status.HTTP_200_OK, response_description="kw4rgs's front cedula extractor")
async def cedula_front_extractor(data: dict):
    try:
        image_data = data.get('data')
        image_bytes = base64.b64decode(image_data)
        extracted_data = dni_extractor_instance.extract_cedula_data(image_bytes)
